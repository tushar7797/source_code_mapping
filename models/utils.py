...
def create_graph(ast, embedding):
    edge_head = []
    edge_tail = []
    node_emb = []
    node_id = 0
    for node in ast:
        node_type = torch.tensor(tokenizer(node['type'])['input_ids']).to('cuda')
        node_type = torch.mean(embedding(node_type),0)
        if 'value' in node.keys():
            node_value = tokenizer(str(node['value']))['input_ids']
            if len(node_value) != 0:
                node_value = torch.tensor(tokenizer(str(node['value']))['input_ids']).to('cuda')
                node_value = torch.mean(embedding(node_value),0)
            else:
                node_value = torch.zeros(embedding.embedding_dim).to('cuda')
        else:
            node_value = torch.zeros(embedding.embedding_dim).to('cuda')
